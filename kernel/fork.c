...
int copy_mem(int nr,struct task_struct * p)
{
	...
	code_limit=get_limit(0x0f); /*从选择子对应的描述符中得到段限长，这里选择子的后三位都是111,所以是从ldt表项中得到限长度*/
	data_limit=get_limit(0x17);
	old_code_base = get_base(current->ldt[1]);/*从ldt表项中取基地址，这里的ldt内容都是在copy_process中从父进程直接复制过来的*/
